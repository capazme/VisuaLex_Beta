...
document.getElementById('scrape-form').addEventListener('submit', async function(e) {
    ...
    try {
        ...
        const normaResponse = await fetch('/create_norma', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(data)
        });
        const normaResult = await normaResponse.json();
        
        if (normaResult.error) {
            handleError(normaResult.error, document.getElementById('result'));
            setLoading(false);
            return;
        }

        const normaData = normaResult.norma_data;
        const tree = normaResult.tree;
        lastUrn = normaResult.urn;
        displayNormaData(normaData, ""); // Clear previous result

        setArticleTree(tree);

        // Estrazione dell'articolo
        const articleResponse = await fetch('/extract_article', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ urn: lastUrn, article: data.article })
        });
        ...
        const brocardiResponse = await fetch('/brocardi_info', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ urn: lastUrn })
        });
        const brocardiResult = await brocardiResponse.json();
